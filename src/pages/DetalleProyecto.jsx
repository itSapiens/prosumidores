...
export default function DetalleProyecto() {
  ...
  async function cargarDatos() {
    setLoading(true)
    const [{ data: inst }, { data: emp }, { data: res }] = await Promise.all([
      supabase.from('installations')
        .select('*, distribuidoras(id, nombre, codigo)')
        .eq('id', id).single(),
      supabase.from('empresa_config').select('*').limit(1).single(),
      supabase.from('installation_reservations')
        .select('*, studies(id, status, customer), clients(id, nombre, apellidos, email, dni)')
  ...
  async function cerrarVersion() {
    ...
    if (!confirm(`¿Cerrar la versión ${versionActiva.version} del Acuerdo de Reparto? Esta acción no se puede deshacer.`)) return
  ...
  async function nuevaVersion() {
    ...
    if (!confirm(`¿Iniciar la versión ${siguienteNum} del Acuerdo de Reparto? Los partícipes y coeficientes actuales formarán el nuevo acuerdo.`)) return
  ...
  async function añadirComoParticipe(reserva) {
    ...
    // Calcular coeficiente a partir de kWp reservados / potencia total instalación
    const coef = instalacion?.potencia_instalada_kwp
      ? parseFloat(((reserva.reserved_kwp / instalacion.potencia_instalada_kwp) * 100).toFixed(4))
      : 0
      ...
      <div style={{ flex: 1, minWidth: 0, transition: 'margin-right .25s', marginRight: panelParticipe ? 400 : 0 }}>
        ...
        <div className="flex-between mb-24">
          ...
          <div className="flex-row gap-8">
            ...
            <button className="btn btn-ghost" onClick={() => navigate(`/proyectos/${id}/editar`)}>Editar</button>
          </div>
        </div>
        ...
        <div className="two-col mb-24">
          <div className="card card-sm">
            ...
            <div className="form-grid" style={{ gap: 10 }}>
              {[
                ['CUPS generador', instalacion.cups_generador, true],
          ...
          <div className="card card-sm">
            ...
            {/* Versión activa del acuerdo */}
            <div style={{ borderTop: '1px solid var(--border)', marginTop: 10, paddingTop: 10 }}>
              {versionActiva ? (() => {
                const firmadosAcuerdo = participes.filter(p => p.doc_acuerdo === 'firmado').length
                const todosF = firmadosAcuerdo === participes.length && participes.length > 0
                return (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
                        Desde {new Date(versionActiva.fecha_inicio).toLocaleDateString('es-ES')} · {firmadosAcuerdo}/{participes.length} firmados
                      </div>
                    </div>
                    {todosF ? (
                      <button className="btn btn-sm btn-primary" onClick={cerrarVersion} disabled={cerrandoVersion}>
                        {cerrandoVersion ? '...' : 'Cerrar v' + versionActiva.version}
                      </button>
                    ) : null}
                  </div>
                )
                ...
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                  Sin versión activa — genera el PDF del acuerdo para iniciar v1
                </div>
              )}
              {!versionActiva && versionesHistorial.length > 0 && (
                <button className="btn btn-sm" style={{ marginTop: 8, width: '100%' }} onClick={nuevaVersion}>
                  + Nueva versión (v{(versionesHistorial.at(-1)?.version || 0) + 1})
                </button>
              )}
            </div>
          </div>
                        ...
                        <div className="actions-cell">
                          {!esParticipe && r.client_id && (
                            <button
                              ...
                              title={`Añadir con coef. ${instalacion?.potencia_instalada_kwp ? ((r.reserved_kwp / instalacion.potencia_instalada_kwp) * 100).toFixed(4) : '?'}%`}
                            >
                              + Partícipe
                            </button>
                          )}
